...
def _spec(max_messages: int = 10) -> ReplySequenceSpec:
    return ReplySequenceSpec(max_messages=max_messages)
...
@pytest.mark.asyncio
async def test_each_plain_chat_line_becomes_one_qq_message(database: Database) -> None:
    runtime = await RuntimeConfigService(
        settings=make_settings(database.url),
        database=database,
    ).snapshot()
...
@pytest.mark.asyncio
async def test_formatted_content_preserves_internal_lines(database: Database) -> None:
    runtime = await RuntimeConfigService(
        settings=make_settings(database.url),
        database=database,
    ).snapshot()
