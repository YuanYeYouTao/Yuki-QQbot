...
def test_release_identity_matches_all_version_surfaces() -> None:
    assert validate_release_identity(ROOT, "v3.6.1") == VERSION


@pytest.mark.parametrize("tag", ["3.6.1", "v3.5", "v3.5.3-rc1", "v03.5.3", "latest"])
...
def test_bundle_contains_only_deployment_files_and_expected_assets(tmp_path: Path) -> None:
    tracked = tracked_files(ROOT) | {"docs/releases/v3.6.1.md", "install.sh", "install.ps1"}
    assets = build_release_bundle(ROOT, tmp_path, VERSION, tracked=tracked)
    assert {path.name for path in assets} == {
        "yuki-3.6.1-deploy.zip",
        "yuki-3.6.1-deploy.tar.gz",
        "docker-compose.yml",
        ".env.example",
        "Yuki-3.6.1-Upgrade.md",
        "install.sh",
        "install.ps1",
        "SHA256SUMS",
    }
    with zipfile.ZipFile(tmp_path / "yuki-3.6.1-deploy.zip") as archive:
        names = set(archive.namelist())
        shell_mode = archive.getinfo("yuki-3.6.1-deploy/install.sh").external_attr >> 16
    prefix = "yuki-3.6.1-deploy/"
    ...
    with tarfile.open(tmp_path / "yuki-3.6.1-deploy.tar.gz", "r:gz") as archive:
...
def test_release_smoke_uses_non_model_genie_import_sentinels(tmp_path: Path) -> None:
    (tmp_path / ".env.example").write_text("YUKI_VERSION=3.6.1\n", encoding="utf-8")
        ...
        def run(self, *arguments: str, capture: bool = False) -> str:
            ...
            if arguments[:4] == ("exec", "-T", "bot", "python"):
                if "urllib.request" in arguments[-1]:
                    return (
                        '{"status":"ok","version":"3.6.1","database":"ok",'
