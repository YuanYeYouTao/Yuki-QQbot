...
- Agent 当前没有隐藏 JSON 最终回复协议。真实使用归因通过受本轮注入白名单约束的终止型
  `finalize_memory_response` 本地响应控制工具完成；工具在同一次模型请求中提交正文和 refs，
  后端直接结束 Agent，并且只有最终正文/语音获得发送回执后才强化。
...
当本轮提供过长期记忆时，Agent 通过后端提供的终止型本地响应控制工具同时提交最终正文和
隐藏使用报告：

```yaml
tool: finalize_memory_response
content: "记得你偏好深烘咖啡。"
memory_refs:
  - M312
```

后端将 `content` 直接作为 Agent 最终正文，不再追加一次模型请求；用户只看到正常回复。
未实质使用记忆时 `memory_refs` 为空。工具必须单独调用，refs 必须来自本轮真正呈现过的白名单。
